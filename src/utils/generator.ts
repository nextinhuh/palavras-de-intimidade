...
export default async function generate(options: Options): Promise<WordSearch> {
  if (options.words == null) options.words = []
  if (options.diagonals == null) options.diagonals = true
  if (options.minLength == null) options.minLength = defaultMinLength
  if (options.maxLength != null && options.maxLength < options.minLength)
    options.maxLength = options.minLength
  if (options.width == null) {
    if (options.height != null) options.width = options.height
    else options.width = defaultSize
  }
  ...
  const { width, height, diagonals } = options
